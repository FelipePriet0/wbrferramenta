-- Fichas arquivadas (archived_at IS NOT NULL) não podem sofrer mutações via RPC.
-- Rollback: 20260527110001_archived_guard_in_rpcs_rollback.sql
...
BEGIN
  ...
  SELECT area, lower(stage), archived_at
    INTO v_old_area, v_old_stage, v_archived
  FROM public.kanban_cards WHERE id = p_card_id FOR UPDATE;

  IF v_old_area IS NULL THEN RAISE EXCEPTION 'card_not_found'; END IF;
  IF v_archived IS NOT NULL THEN RAISE EXCEPTION 'card_archived'; END IF;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.kanban_cards WHERE id = p_card_id AND archived_at IS NOT NULL) THEN
...
BEGIN
  ...
  SELECT urgent_reason_by, archived_at INTO v_author, v_archived
