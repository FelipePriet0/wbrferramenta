-- Abre o caminho de revert de cancelamento na RPC change_stage pra
-- qualquer role autenticada não-leitor (antes era apenas analista/gestor).
-- Motivo de reversão continua sendo obrigatório — apenas o gate de role foi
-- removido. Vendedores e instaladores agora podem reverter cancelamentos,
-- tanto via drag&drop (RevertCancelModal) quanto via remoção da etiqueta
-- Cancelada (ReverterPelaEtiquetaModal).
...
BEGIN
  ...
  -- Cancellation (preserves source area; reason required).
  ...
  -- Revert from Canceladas — now open to any non-leitor role.
  -- Motivo continues to be required for audit; the role gate was removed.
  ...
  -- Ingress: recebidos → preenchidas (auto-assigns).
