-- Estreita o RBAC do revert: instalador deixa de poder reverter cancelamento.
-- Decisão: instalador atua na ponta da operação (pós-aprovação), então não
-- faz sentido ele "des-cancelar" uma ficha — quem cancelou (vendedor/análise)
-- ou um gestor é quem deve assumir a reversão.
--
-- Top-level gate da função continua aceitando os 4 roles (vendedor, analista,
-- gestor, instalador) pra todos os outros movimentos. Apenas o bloco de
-- revert ganhou um IF NOT user_has_role(['vendedor','analista','gestor'])
-- adicional. Motivo continua obrigatório.
...
BEGIN
  ...
  -- Revert: instalador excluído. Motivo continua obrigatório pra audit.
