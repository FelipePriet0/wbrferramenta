-- Adiciona regra à RPC change_stage: voltar pra coluna 'recebidos' (vinda de
-- qualquer outro stage) desatribui o analista atual. O próximo analista a
-- clicar na ficha vira o novo assignee.
--
-- O caminho de promoção comercial → analise/recebidos (p_area=comercial,
-- v_new_stage=concluidas) já estava zerando assignee_id; esta migration
-- estende a regra a TODOS os caminhos que terminam em recebidos.
...
BEGIN
  ...
  -- Cancellation: needs a reason; lands in Canceladas of Análise.
  ...
  -- Bug 11: voltar pra Recebidos vindo de qualquer outro stage desatribui
  -- o analista atual. O próximo analista a clicar na ficha vira o novo
  -- assignee. O caminho de promoção comercial→recebidos acima também zera
  -- assignee_id, então esta regra só captura os caminhos de "volta".
  ...
  -- Finalization. Only the three terminal columns can move into Finalizados.
