...
export function CalendarReady({ label, value, placeholder, disabled, disablePast, onChange }: CalendarReadyProps) {
  const [open, setOpen] = React.useState(false);
  const selectedDate = React.useMemo(() => parseDateOnly(value), [value]);

  const formatted = React.useMemo(() => {
    if (!selectedDate) return placeholder || "Selecionar data";
      ...
      <Popover open={open} onOpenChange={(next) => !disabled && setOpen(next)}>
        ...
        <PopoverContent className="w-auto rounded-xl border-0 bg-white p-0 shadow-md dark:bg-zinc-950" align="start" sideOffset={8}>
          <DayPicker
            ...
            modifiersClassNames={{
              today: "border border-emerald-400 text-emerald-600 font-semibold",
            }}
          />
        </PopoverContent>
      </Popover>
