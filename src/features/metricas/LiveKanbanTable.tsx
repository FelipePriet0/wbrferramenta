...
type Props = { area: KanbanArea };

const COL_LABEL: Record<KanbanArea, { feitas: string; aguardando: string; canceladas: string }> = {
  comercial: { feitas: 'Feitas', aguardando: 'Aguard. Docs', canceladas: 'Canceladas' },
  analise:   { feitas: 'Em Análise', aguardando: 'Reanálise', canceladas: 'Negadas' },
};

export function LiveKanbanTable({ area }: Props) {
  const [rows, setRows] = useState<GamificacaoRow[]>([]);
  const [loading, setLoading] = useState(true);
  const cols = COL_LABEL[area];

  const refresh = useCallback(async () => {
    try {
      const data = await getLivePipelineData(area);
      setRows(data);
    } catch (e) {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-zinc-100">
              <th className="px-4 py-2 text-left text-[11px] font-semibold uppercase tracking-wide text-zinc-400">
                #
              </th>
              <th className="px-4 py-2 text-left text-[11px] font-semibold uppercase tracking-wide text-zinc-400">
                Colaborador
              </th>
              <th className="px-4 py-2 text-center text-[11px] font-semibold uppercase tracking-wide text-zinc-400">
                {cols.feitas}
              </th>
              <th className="px-4 py-2 text-center text-[11px] font-semibold uppercase tracking-wide text-zinc-400">
                {cols.aguardando}
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-xs text-zinc-400">
                  Nenhum colaborador com fichas ativas.
                </td>
              </tr>
            ) : (
              rows.map((row, i) => (
