/**
 * Config do formulário do modelo `altplan-sem-troca-visita-paga` (coluna
 * esquerda do gerador). Campos, seções, opções e visibilidade condicional
 * (`mostrarQuando`), extraídos do bundle legado e de `sondagem2-suporte.json`.
 * Sem troca de equipamento, com visita técnica paga.
 */
...
const ehTerceiroSolicita = (v: Valores) =>
  TERCEIRO_SOLICITA.includes(v.tipoSolicitacao ?? '');
const temAutorizado = (v: Valores) =>
  v.tipoSolicitacao === 'titular-solicita-terceiro-acompanha';
const temParente = (v: Valores) => COM_PARENTE.includes(v.tipoSolicitacao ?? '');

export const ALTPLAN_SEM_TROCA_VISITA_PAGA_FORM: ModeloForm = {
  ...
  secoes: [
    ...
    },
    {
      ...
      campos: [
        ...
        { id: 'autorizado', label: 'Terceiro autorizado', controle: 'text', placeholder: 'Quem vai acompanhar/assinar', span: 6, mostrarQuando: temAutorizado },
