...
export const PRAZO_ATE_90 = 'ATE_90'
export const PRAZO_MAIS_90 = 'MAIS_90'

/**
 * Texto de análise para o gestor do suporte (Hiago) quando o cliente
 * contratou dentro de 90 dias — nesses casos a mudança não é feita direto,
 * precisa passar por essa análise antes. Vazio quando "+ 90 dias".
 * Estilo de frase (não CAIXA ALTA como o resto) — é uma mensagem interna,
 * não um texto de O.S.
 */
export function texto90Dias(
  ...
  if (v.prazoContratacao !== PRAZO_ATE_90) return ''
  const enderecoLinha = [adress, num].filter(Boolean).join(', ') + (complemento ? ` (${complemento})` : '')
  return `ANALISE
${cliente}
> Mudança de endereço (${tipoLabel})
> Dentro do prazo de 90 dias.
> Agendado para dia ${v.dataVisita ?? ''}.
> Novo endereço: ${enderecoLinha}
> CEP: ${v.cep ?? ''} / Bairro: ${bairro}
> Protocolo: ${v.protocolo ?? ''}`
}

/** Anexa as Observações preenchidas pelo operador ao fim do texto de protocolo. */
export function comObservacoes(texto: string, observacoes: string): string {
...
export function buildMudEndPadraoTextos(
  ...
  const texto90 = texto90Dias(v, cliente, adress, num, complemento, bairro, 'Padrão')
