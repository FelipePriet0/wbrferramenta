...
export function buildMudEndAltplanPropostaTextos(
  ...
  const contatoUsado =
    tipo === T_TERCEIRO_TITULAR || tipo === T_TERCEIRO_TERCEIRO
      ? contatoSol
      : contato
  const aberturaContato =
    tipo === T_TERCEIRO_TITULAR || tipo === T_TERCEIRO_TERCEIRO
      ? `${solicitantePrimeiro} (${parente} DE ${clientePrimeiro})`
      : primeiroContato
  const partesLigacao = String(v.dataLigacao ?? '').trim().split(/\s+/)
  const dataLigacao = partesLigacao[0] ?? ''
  const horaLigacao = partesLigacao[1] ?? ''
    ...
    tipo === T_TERCEIRO_TITULAR
      ...
      : tipo === T_TERCEIRO_TERCEIRO
        ...
        : tipo === T_TITULAR_TERCEIRO
          ? `\n\n${clientePrimeiro} DISSE QUE NÃO ESTARÁ PRESENTE, MAS AUTORIZOU ${autorizado} (${parente}) A ACOMPANHAR E ASSINAR O.S.\n${clientePrimeiro} INFORMOU O NÚMERO DE CONTATO (${contatoAut}) PARA CASO SEJA NECESSÁRIO FALAR COM ${autorizadoPrimeiro}.`
          : ''

  const protocolo = `${aberturaContato} ENTROU EM CONTATO POR ${v.canal ?? ''} (${contatoUsado}) E PEDIU INFORMAÇÕES SOBRE MUDANÇA DE ENDEREÇO.
...
${primeiroContato} CONCORDOU COM OS TERMOS DE ALTERAÇÃO, ESTÁ CIENTE DA RENOVAÇÃO DA FIDELIDADE.${protocoloExtra}
...
NOME NO COMPROVANTE: ${nomeComprov} (${grauComp})`
  ...
  const osPresenca =
    tipo === T_TITULAR_TERCEIRO
      ? `${clientePrimeiro} DISSE QUE NÃO ESTARÁ PRESENTE, MAS AUTORIZOU ${upper(v.autorizado)} (${parente}) A ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO.`
      : tipo === T_TERCEIRO_TERCEIRO
        ? `POR PROCEDIMENTO PADRÃO ENTREI EM CONTATO POR ${v.canalTit ?? ''} (${contato}) DIA ${dataLigacao} ÀS ${horaLigacao}HRS COM ${clientePrimeiro} (ASSINANTE) QUE CONFIRMOU E AUTORIZOU ${solicitante} (${parente}) ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO.`
        : tipo === T_TERCEIRO_TITULAR
          ? `POR PROCEDIMENTO PADRÃO ENTREI EM CONTATO POR ${v.canal ?? ''} (${contato}) COM ${clientePrimeiro} (ASSINANTE) QUE CONFIRMOU E DISSE QUE ESTARÁ PRESENTE PARA ACOMPANHAR, ASSINAR O.S E CONTRATO.`
...
${textoTecnico(v, troca)}`
  ...
  const texto90 = texto90Dias(v, cliente, adress, num, complemento, bairro, 'Alt Plano Isento')
...
export const MUD_END_ALTPLAN_PROPOSTA_FIELDS: OsTemplateField[] = [
  ...
  { id: 'observacoes', label: 'Observações', control: 'textarea', placeholder: 'Anotações gerais sobre o atendimento…', section: S_ID, layout: { md: 12 } },
  ...
  { id: 'quandoMud', label: 'Quando vai se mudar', control: 'text', section: S_END, showWhen: { field: 'tipoSolicitacao', equals: [T_TITULAR, T_TITULAR_TERCEIRO] }, layout: { md: 12 } },
  {
    id: 'mudou',
