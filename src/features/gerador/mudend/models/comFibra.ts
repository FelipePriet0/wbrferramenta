...
/**
 * MUD END — COM FIBRA EXISTENTE (wbr pré-existente no novo endereço).
...
function protCustoFibra(nome: string): string {
  return `INFORMEI A ${nome} QUE POSSUÍMOS VIABILIDADE DE FIBRA ÓTICA NO ENDEREÇO INFORMADO, E ${nome} DISSE QUE NA RESIDÊNCIA JÁ POSSUI FIBRA DA WBR.
...
const DISSE_DROP =
  'DISSE "QUE MUDOU PARA ESTE ENDEREÇO, LEVOU OS EQUIPAMENTOS E QUE A CASA JÁ POSSUI DROP DA WBR INSTALADO".'
...
export function buildMudEndComFibraTextos(
  ...
  const texto90 = texto90Dias(v, cliente, adress, num, complemento, bairro, 'Com Fibra WBR')
...
MUDANÇA AGENDADA PARA DIA ${v.dataVisita ?? ''} ${v.horaVisita ?? ''} HRS.`
  ...
  const os = osBody(
    `${clientePrimeiro} ENTROU EM CONTATO VIA ${v.canal ?? ''} (${contato}) E SOLICITOU REINSTALAÇÃO DOS EQUIPAMENTOS DE INTERNET NO ENDEREÇO QUE ESTÁ NA O.S, DISSE QUE MUDOU PARA ESTE ENDEREÇO, LEVOU OS EQUIPAMENTOS E QUE A CASA JÁ POSSUI DROP DA WBR INSTALADO. ${osCustoFibra(v.formaPag ?? '')}  ${clientePrimeiro} DISSE QUE ESTARÁ PRESENTE PARA ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO. VISITA AGENDADA PARA ${v.dataVisita ?? ''} ${v.horaVisita ?? ''}HR.`,
    v,
  )
  return {
    mudEndTextoProtocolo: protocolo,
