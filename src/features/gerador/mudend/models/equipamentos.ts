...
function protocoloBase({
  ...
  dataVisita: string
  horaVisita: string
  extraAntesAgendamento?: string
}): string {
  return `${abertura ?? quemPrimeiro} ENTROU EM CONTATO POR ${canal} (${contato}) E PEDIU INFORMAÇÕES SOBRE MUDANÇA DE ENDEREÇO.
...
export function buildMudEndEquipamentosTextos(
  ...
  const texto90 = texto90Dias(v, cliente, adress, num, complemento, bairro, 'Buscar Equipamentos')

  if (tipo === T_TERCEIRO_TITULAR) {
    const protocolo = protocoloBase({
      quemPrimeiro: solicitantePrimeiro,
      abertura: `${solicitantePrimeiro} (${parenteSol} DE ${clientePrimeiro})`,
      canal: v.canal ?? '',
      contato: contatoSol,
      signalLine,
      adress,
      ...
      extraAntesAgendamento: `POR PROCEDIMENTO PADRÃO, ENTREI EM CONTATO POR ${v.canal ?? ''} (${contato}) COM ${clientePrimeiro} (ASSINANTE) QUE AUTORIZOU A SOLICITAÇÃO E CONFIRMOU QUE ESTARÁ PRESENTE NO ATO DA VISITA.`,
    })
  ...
  if (tipo === T_TERCEIRO_TERCEIRO) {
    ...
    const protocolo = protocoloBase({
      ...
      dataVisita: v.dataVisita ?? '',
      horaVisita: v.horaVisita ?? '',
      extraAntesAgendamento: `POR PROCEDIMENTO PADRÃO, ENTREI EM CONTATO POR ${v.canalTit ?? ''} (${contato}) DIA ${dataLigacao} ÀS ${horaLigacao}HRS COM ${clientePrimeiro} (ASSINANTE) QUE CONFIRMOU E AUTORIZOU ${solicitante} (${parenteSol}) ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO.`,
    })
  ...
  if (tipo === T_TITULAR_TERCEIRO) {
    const protocolo = protocoloBase({
      ...
      dataVisita: v.dataVisita ?? '',
      horaVisita: v.horaVisita ?? '',
      extraAntesAgendamento: `${clientePrimeiro} DISSE QUE NÃO ESTARÁ PRESENTE, MAS AUTORIZOU ${autorizado} (${parente}) A ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO.`,
    })
...
export const MUD_END_EQUIPAMENTOS_FIELDS: OsTemplateField[] = [
  ...
  },
  {
    ...
    control: 'text',
    placeholder: 'Ex.: -12.34DBM ou SEM SINAL',
    section: S_ID,
    layout: { md: 3 },
  },
  {
