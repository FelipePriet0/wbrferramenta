...
function esperado(v: Entrada) {
  ...
  const sinalSaida = String(v.sinalONU ?? '').trim().toUpperCase()
...
${SEP_AST}
${IND4}
INFORMEI A ${c0} QUE POSSUÍMOS VIABILIDADE DE FIBRA ÓTICA NO ENDEREÇO INFORMADO, E ${c0} DISSE QUE NA RESIDÊNCIA JÁ POSSUI FIBRA DA WBR.
...
function buildContext(v: Entrada, extra: Record<string, unknown> = {}): Record<string, unknown> {
  const base = {
    ...v,
    ...extra,
...
function gerarNovo(v: Entrada, extra: Record<string, unknown> = {}) {
  const full = renderTemplate(MUD_END_COM_FIBRA_OUTPUT, buildContext(v, extra))
  const secs = splitOsPreviewSections(full)
  return {
    protocolo: secs[0]?.body ?? '',
    os: secs[1]?.body ?? '',
    agenda: secs[2]?.body ?? '',
    texto90Dias: secs[3]?.body ?? '',
  }
}
...
describe('MUD END com fibra existente', () => {
  ...
  it('Terceiro: protocolo menciona DROP da WBR e o solicitante', () => {
    ...
    expect(secs[1]?.body ?? '').toContain('DROP DA WBR')
  })
})

describe('MUD END com fibra existente — Texto 90 dias', () => {
  it('fica vazio quando "+ 90 dias" (ou campo não preenchido)', () => {
    expect(gerarNovo(CENARIO).texto90Dias).toBe('')
    expect(gerarNovo(CENARIO, { prazoContratacao: 'MAIS_90' }).texto90Dias).toBe('')
  })

  it('gera o texto de análise quando "90 dias"', () => {
    const texto = gerarNovo(CENARIO, { prazoContratacao: 'ATE_90' }).texto90Dias
    expect(texto).toBe(
      `ANALISE
JOÃO DA SILVA
> Mudança de endereço (Com Fibra WBR)
> Dentro do prazo de 90 dias.
> Agendado para dia 15/06/2026.
> Novo endereço: AVENIDA DOS EUCALIPTOS, 624 (CASA FUNDOS)
> CEP: 38400000 / Bairro: SARAIVA
> Protocolo: 123.456`,
    )
  })
})
