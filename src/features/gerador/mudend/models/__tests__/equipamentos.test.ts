...
function esperado(v: Entrada) {
  ...
  const signalLine = `${equipPrefix} ${String(v.sinalONU ?? '').trim().toUpperCase()}`
  const operador =
...
describe('MUD END buscar equipamentos — Texto 90 dias', () => {
  ...
  it('gera o texto de análise quando "90 dias"', () => {
    ...
    expect(texto).toBe(
      `ANALISE
JOÃO DA SILVA
> Mudança de endereço (Buscar Equipamentos)
