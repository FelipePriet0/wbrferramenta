...
function esperado(v: Entrada) {
  ...
  const sinal = String(v.sinalONU ?? '').trim().toUpperCase()
...
function gerar(v: Entrada, tipo: string, extra: Record<string, unknown> = {}) {
  const operadorPrimeiroNome =
    v.operadorDisplayName.trim().split(/\s+/).filter(Boolean)[0]?.toUpperCase() ??
    ''
  const base = { ...v, ...extra, tipoSolicitacao: tipo, operadorPrimeiroNome }
...
describe('MUD END + Alt Plano pago — Texto 90 dias', () => {
  it('fica vazio quando "+ 90 dias" (ou campo não preenchido)', () => {
    expect(gerar(CENARIO, T_TITULAR).texto90Dias).toBe('')
    expect(gerar(CENARIO, T_TITULAR, { prazoContratacao: 'MAIS_90' }).texto90Dias).toBe('')
  })

  it('gera o texto de análise quando "90 dias"', () => {
    const texto = gerar(CENARIO, T_TITULAR, { prazoContratacao: 'ATE_90' }).texto90Dias
    expect(texto).toBe(
      `ANALISE
JOÃO DA SILVA
> Mudança de endereço (Alt Plano Pago)
