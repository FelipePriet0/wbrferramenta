...
function gerarNovo(v: Entrada, extra: Record<string, unknown> = {}) {
  const full = renderTemplate(MUD_END_PADRAO_OUTPUT, buildContext(v, extra))
...
describe('MUD END padrão — Texto 90 dias', () => {
  it('fica vazio quando "+ 90 dias" (ou campo não preenchido)', () => {
    expect(gerarNovo(CENARIO_A).texto90Dias).toBe('')
    expect(gerarNovo(CENARIO_A, { prazoContratacao: 'MAIS_90' }).texto90Dias).toBe('')
  })

  it('gera o texto de análise quando "90 dias"', () => {
    const texto = gerarNovo(CENARIO_A, { prazoContratacao: 'ATE_90' }).texto90Dias
    expect(texto).toBe(
      `ANALISE
JOÃO DA SILVA
> Mudança de endereço (Padrão)
