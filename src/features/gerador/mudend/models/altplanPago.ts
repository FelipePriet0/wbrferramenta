...
/**
 * MUD END + ALT PLANO (pago — cobra R$100,00 do serviço).
 * Origem (4 variações em um único fluxo):
 *  - index-mud-altplan-pago.html ........... titular solicita e acompanha
 *  - mud-altplan-pago3.html ................ titular solicita e autoriza terceiro
 *  - mud-altplan-pago1.html ................ terceiro solicita e terceiro acompanha
 *  - mud-altplan-pago2.html ................ terceiro solicita e titular acompanha
 */
...
function osBody(intro: string, v: Record<string, string>, trocaAgenda: string, troca: string): string {
  return `${intro}
${trocaAgenda}
...
export function buildMudEndAltplanPagoTextos(
  ...
  const sinalSaida = upper(v.sinalONU)
  ...
  const agenda = `MUD END + ALT PLANO ${cliente} PROT:${v.protocolo ?? ''} ${v.formaPag ?? ''} (${operadorPrimeiroNome}) - ${bairro} ${v.prumada ?? ''} // ${extendAgenda} ${trocaAgenda}`
  const texto90 = texto90Dias(v, cliente, adress, num, complemento, bairro, 'Alt Plano Pago')

  // Cabeçalho/protagonista do protocolo conforme a variação.
  const ehTerceiro = tipo === T_TERCEIRO_TITULAR || tipo === T_TERCEIRO_TERCEIRO
  const proto0 = ehTerceiro ? s0 : c0
  const header = ehTerceiro
    ? `${s0} (${parente} DE ${c0}) ENTROU EM CONTATO POR ${v.canal ?? ''} (${contatoSol})`
    : `${c0} ENTROU EM CONTATO POR ${v.canal ?? ''} (${contato})`

  // Miolo comum (idêntico nas 4 variações, exceto o protagonista).
  const miolo = `${header} E PEDIU INFORMAÇÕES SOBRE MUDANÇA DE ENDEREÇO.
...
QUESTIONADO, ${proto0} DISSE QUE ${v.mudou ?? ''} DESEJA QUE OS EQUIPAMENTOS SEJAM REINSTALADOS NO NOVO ENDEREÇO.
...
INFORMEI A ${proto0} QUE POSSUÍMOS VIABILIDADE DE FIBRA ÓTICA NO ENDEREÇO INFORMADO E QUE PARA REALIZAÇÃO DA NOVA INSTALAÇÃO COBRAMOS O VALOR DE SERVIÇO R$100,00 A SER PAGO PARA O TÉCNICO NO ATO (EM DINHEIRO, CARTÃO OU PIX).

${equipSituacao}

${SEP}

NA OPORTUNIDADE, OFERTEI ALTERAÇÃO DE PLANO PARA ${proto0}.
...
FORMA DE PAGAMENTO EM ${v.formaPag ?? ''}. ${c0} DISSE QUE NÃO ESTARÁ PRESENTE, MAS AUTORIZOU ${autorizado} (${parente}) A ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO.
...
MUDANÇA E ALTERAÇÃO DE PLANO AGENDADA (A PEDIDO DO CLIENTE) PARA DIA ${v.dataVisita ?? ''} ${v.horaVisita ?? ''} HRS`
    os = osBody(
      `${c0} ENTROU EM CONTATO VIA ${v.canal ?? ''} (${contato}) E SOLICITOU REINSTALAÇÃO DOS EQUIPAMENTOS DE INTERNET NO ENDEREÇO QUE ESTÁ NA O.S, DISSE "QUE MUDOU PARA ESTE ENDEREÇO E LEVOU OS EQUIPAMENTOS". VALOR DO SERVIÇO: R$100,00. SOLICITOU TAMBÉM A RENOVAÇÃO DO SEU CONTRATO E UPGRADE DE PLANO, PLANO ATUAL: ${v.planoAtual ?? ''}. PLANO ESCOLHIDO: ${v.planoEscolhido ?? ''}. CIENTE QUE A ALTERAÇÃO DE PLANO RENOVA O CONTRATO DE PERMANÊNCIA PARA 12 (DOZE) MESES A PARTIR DA ASSINATURA DA O.S E CONTRATO. ${c0} DISSE QUE IRÁ REALIZAR O PAGAMENTO EM ${v.formaPag ?? ''}. ${c0} DISSE QUE NÃO ESTARÁ PRESENTE, MAS AUTORIZOU ${autorizado} (${parente}) A ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO. VISITA AGENDADA (A PEDIDO DO CLIENTE) PARA ${v.dataVisita ?? ''} ${v.horaVisita ?? ''} HRS.`,
      v,
      trocaAgenda,
      troca,
    )
    return { mudEndTextoProtocolo: comObservacoes(protocolo, v.observacoes ?? ''), mudEndTextoOS: os, mudEndTextoAgenda: agenda, mudEndTexto90Dias: texto90 }
  }
...
FORMA DE PAGAMENTO EM ${v.formaPag ?? ''}. POR PROCEDIMENTO PADRÃO, ENTREI EM CONTATO POR ${v.canalTit ?? ''} (${contato}) COM ${c0} (ASSINANTE) QUE CONFIRMOU E AUTORIZOU ${solicitante} (${parente}) ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO. MUDANÇA E ALTERAÇÃO DE PLANO AGENDADA PARA DIA ${v.dataVisita ?? ''} ${v.horaVisita ?? ''} HRS.`
    os = osBody(
      `${s0} (${parente} DE ${c0}) ENTROU EM CONTATO POR ${v.canal ?? ''} (${contatoSol}) E SOLICITOU REINSTALAÇÃO DOS EQUIPAMENTOS DE INTERNET NO ENDEREÇO QUE ESTÁ NA O.S, DISSE "QUE MUDOU PARA ESTE ENDEREÇO E LEVOU OS EQUIPAMENTOS". VALOR DO SERVIÇO: R$100,00. SOLICITOU TAMBÉM A RENOVAÇÃO DO SEU CONTRATO E UPGRADE DE PLANO, PLANO ATUAL: ${v.planoAtual ?? ''}. PLANO ESCOLHIDO: ${v.planoEscolhido ?? ''}. CIENTE QUE A ALTERAÇÃO DE PLANO RENOVA O CONTRATO DE PERMANÊNCIA PARA 12 (DOZE) MESES A PARTIR DA ASSINATURA DA O.S E CONTRATO. ${s0} DISSE QUE ESTARÁ PRESENTE PARA ACOMPANHAR, ASSINAR O.S E CONTRATO E IRÁ REALIZAR O PAGAMENTO EM ${v.formaPag ?? ''}. POR PROCEDIMENTO PADRÃO ENTREI EM CONTATO POR ${v.canalTit ?? ''} (${contato}) COM ${c0} (ASSINANTE) QUE CONFIRMOU E AUTORIZOU ${solicitante} (${parente}) ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO. VISITA AGENDADA (A PEDIDO DO CLIENTE) PARA ${v.dataVisita ?? ''} ${v.horaVisita ?? ''} HRS.`,
      v,
      trocaAgenda,
      troca,
    )
...
FORMA DE PAGAMENTO EM ${v.formaPag ?? ''}. POR PROCEDIMENTO PADRÃO, ENTREI EM CONTATO POR ${v.canal ?? ''} (${contato}) COM ${c0} (ASSINANTE) QUE CONFIRMOU E DISSE QUE ESTARÁ PRESENTE PARA ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO. ${c0} CONCORDOU COM OS TERMOS DE ALTERAÇÃO, ESTÁ CIENTE DA RENOVAÇÃO DA FIDELIDADE. MUDANÇA E ALTERAÇÃO DE PLANO AGENDADA (A PEDIDO DO CLIENTE) PARA DIA ${v.dataVisita ?? ''} ${v.horaVisita ?? ''} HRS`
    os = osBody(
      `${s0} (${parente} DE ${c0}) ENTROU EM CONTATO POR ${v.canal ?? ''} (${contatoSol}) E SOLICITOU REINSTALAÇÃO DOS EQUIPAMENTOS DE INTERNET NO ENDEREÇO QUE ESTÁ NA O.S, DISSE "QUE MUDOU PARA ESTE ENDEREÇO E LEVOU OS EQUIPAMENTOS". VALOR DO SERVIÇO: R$100,00. SOLICITOU TAMBÉM A RENOVAÇÃO DO SEU CONTRATO E UPGRADE DE PLANO, PLANO ATUAL: ${v.planoAtual ?? ''}. PLANO ESCOLHIDO: ${v.planoEscolhido ?? ''}. CIENTE QUE A ALTERAÇÃO DE PLANO RENOVA O CONTRATO DE PERMANÊNCIA PARA 12 (DOZE) MESES A PARTIR DA ASSINATURA DA O.S E CONTRATO. POR PROCEDIMENTO PADRÃO ENTREI EM CONTATO POR ${v.canal ?? ''} (${contato}) COM ${c0} (ASSINANTE) QUE CONFIRMOU E DISSE QUE ESTARÁ PRESENTE PARA ACOMPANHAR, ASSINAR O.S E EFETUAR O PAGAMENTO EM ${v.formaPag ?? ''}. VISITA AGENDADA (A PEDIDO DO CLIENTE) PARA ${v.dataVisita ?? ''} ${v.horaVisita ?? ''} HRS.`,
      v,
      trocaAgenda,
      troca,
    )
  ...
  // T_TITULAR (padrão): titular solicita e acompanha.
  protocolo = `${miolo}
...
export const MUD_END_ALTPLAN_PAGO_FIELDS: OsTemplateField[] = [
  ...
  { id: 'sinalONU', label: 'Sinal da fibra', control: 'text', placeholder: 'Ex.: -12.34DBM ou SEM SINAL', section: S_ID, layout: { md: 3 } },
  ...
  },
  {
    ...
    section: S_TERCEIRO,
    showWhen: { field: 'tipoSolicitacao', equals: [T_TERCEIRO_TITULAR, T_TERCEIRO_TERCEIRO] },
    layout: { md: 5 },
  },
  {
    id: 'contatoSol',
  ...
  },
  {
    ...
    section: S_TERCEIRO,
    showWhen: { field: 'tipoSolicitacao', equals: [T_TERCEIRO_TITULAR, T_TERCEIRO_TERCEIRO, T_TITULAR_TERCEIRO] },
    layout: { md: 4 },
  },
  ...
  { id: 'quandoMud', label: 'Quando vai se mudar', control: 'text', placeholder: 'Cliente informou que vai mudar na próxima semana.', section: S_END, layout: { md: 12 } },
  {
    id: 'mudou',
  ...
  { id: 'tipoComp', label: 'Informe o tipo (se “Outros”)', control: 'text', placeholder: 'Informe o tipo do comprovante', section: S_COMP, showWhen: { field: 'comprovante', equals: 'OUTROS' }, layout: { md: 3 } },
  { id: 'nomeComprov', label: 'Nome no comprovante', control: 'text', placeholder: 'Nome completo', section: S_COMP, layout: { md: 3 } },
  ...
  {
    id: 'prazoContratacao',
    label: 'Contratou dentro de:',
    control: 'radio',
    defaultValue: PRAZO_MAIS_90,
    section: S_AGE,
    options: [
      { value: PRAZO_ATE_90, label: '90 dias' },
      { value: PRAZO_MAIS_90, label: '+ 90 dias' },
    ],
    layout: { md: 12 },
  },
...
export function getMudEndAltplanPagoDefaults() {
  return {
    ...
    title: 'Mudança de endereço — alt plano pago',
    outputTemplate: MUD_END_ALTPLAN_PAGO_OUTPUT,
    demandCategory: 'mudanca-endereco',
    fields: MUD_END_ALTPLAN_PAGO_FIELDS.map((f) => ({ ...f })),
  }
}
