...
import { renderFeedbackStbRoku } from './feedbackStbRoku';
import fixtures from './__fixtures__/feedbackStbRoku.fixtures.json';
...
describe('renderFeedbackStbRoku × legado (diff)', () => {
  for (const fx of fixtures as Fixture[]) {
    it(`bate no caso ${fx.nome}`, () => {
      const saida = renderFeedbackStbRoku(fx.input);
