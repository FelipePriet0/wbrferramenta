...
import { renderFeedbackMudancaPonto } from './feedbackMudancaPonto';
import fixtures from './__fixtures__/feedbackMudancaPonto.fixtures.json';
...
describe('renderFeedbackMudancaPonto × legado (diff)', () => {
  for (const fx of fixtures as Fixture[]) {
    it(`bate no caso ${fx.nome}`, () => {
      const saida = renderFeedbackMudancaPonto(fx.input);
