...
import { renderMidiaRokuPadrao } from './midiaRokuPadrao';
import fixtures from './__fixtures__/midiaRokuPadrao.fixtures.json';
...
describe('renderMidiaRokuPadrao × legado (diff)', () => {
  for (const fx of fixtures as Fixture[]) {
    it(`bate no caso ${fx.nome}`, () => {
      const saida = renderMidiaRokuPadrao(fx.input);
