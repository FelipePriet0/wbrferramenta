...
import { renderEnceAltplanRemoto } from './enceAltplanRemoto';
import fixtures from './__fixtures__/enceAltplanRemoto.fixtures.json';
...
describe('renderEnceAltplanRemoto × legado (diff)', () => {
  for (const fx of fixtures as Fixture[]) {
    it(`bate no caso ${fx.nome}`, () => {
      const saida = renderEnceAltplanRemoto(fx.input);
