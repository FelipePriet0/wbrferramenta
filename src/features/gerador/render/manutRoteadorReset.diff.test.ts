...
import { renderManutRoteadorReset } from './manutRoteadorReset';
import fixtures from './__fixtures__/manutRoteadorReset.fixtures.json';
...
describe('renderManutRoteadorReset × legado (diff)', () => {
  for (const fx of fixtures as Fixture[]) {
    it(`bate no caso ${fx.nome}`, () => {
      const saida = renderManutRoteadorReset(fx.input);
