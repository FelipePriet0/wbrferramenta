...
import { renderInstGratisEmpresarial } from './instGratisEmpresarial';
import fixtures from './__fixtures__/instGratisEmpresarial.fixtures.json';
...
describe('renderInstGratisEmpresarial × legado (diff)', () => {
  for (const fx of fixtures as Fixture[]) {
    it(`bate no caso ${fx.nome}`, () => {
      const saida = renderInstGratisEmpresarial(fx.input);
