...
import { renderInstGratisResidencial } from './instGratisResidencial';
import fixtures from './__fixtures__/instGratisResidencial.fixtures.json';
...
describe('renderInstGratisResidencial × legado (diff)', () => {
  for (const fx of fixtures as Fixture[]) {
    it(`bate no caso ${fx.nome}`, () => {
      const saida = renderInstGratisResidencial(fx.input);
