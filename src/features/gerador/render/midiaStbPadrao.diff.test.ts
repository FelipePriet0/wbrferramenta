...
import { renderMidiaStbPadrao } from './midiaStbPadrao';
import fixtures from './__fixtures__/midiaStbPadrao.fixtures.json';
...
describe('renderMidiaStbPadrao × legado (diff)', () => {
  for (const fx of fixtures as Fixture[]) {
    it(`bate no caso ${fx.nome}`, () => {
      const saida = renderMidiaStbPadrao(fx.input);
