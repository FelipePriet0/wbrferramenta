...
import { renderEncePadraoCasa } from './encePadraoCasa';
import fixtures from './__fixtures__/encePadraoCasa.fixtures.json';
...
describe('renderEncePadraoCasa × legado (diff)', () => {
  for (const fx of fixtures as Fixture[]) {
    it(`bate no caso ${fx.nome}`, () => {
      const saida = renderEncePadraoCasa(fx.input);
