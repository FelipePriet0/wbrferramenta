...
import { renderWifiExtendTplink } from './wifiExtendTplink';
import fixtures from './__fixtures__/wifiExtendTplink.fixtures.json';
...
describe('renderWifiExtendTplink × legado (diff)', () => {
  for (const fx of fixtures as Fixture[]) {
    it(`bate no caso ${fx.nome}`, () => {
      const saida = renderWifiExtendTplink(fx.input);
